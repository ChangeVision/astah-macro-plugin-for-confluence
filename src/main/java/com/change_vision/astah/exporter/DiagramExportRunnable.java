...
public class DiagramExportRunnable implements Runnable {
	...
	public DiagramExportRunnable(final Attachment attachment, final String astahBase, final String outputBase){
		if(attachment == null) {
	...
	public void run() {
		...
		if(!output.exists()){
			logger.error("Can't create output directory. May be permission issue.'" + output.getAbsolutePath() + "'");
			return;
		}
	...
	private String[] createExportCommand(File file, File javaCommand,
			String outputRoot) {
	...
	private String getOutputRoot() {
		return OUTPUT_BASE  + File.separator + String.valueOf(attachment.getId()) + File.separator + String.valueOf(attachment.getVersion());
	}
